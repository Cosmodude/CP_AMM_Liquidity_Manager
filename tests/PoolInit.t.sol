...
contract PoolInitTest is Test {
    ...
    function test_PoolInit_DeployTokensAndAddLiquidity() public {
        ...
        uint256 expectedMintAmount = 500_000_000_000_000_000; // 0.5 tokens with 18 decimals (remaining after adding liquidity)
        assertEq(tokenA.balanceOf(testUser), expectedMintAmount, "User should have remaining tokens A after adding liquidity");
        assertEq(tokenB.balanceOf(testUser), expectedMintAmount, "User should have remaining tokens B after adding liquidity");
        ...
        assertTrue(reserve0 >= expectedLiquidityAmount && reserve1 >= expectedLiquidityAmount, "Both reserves should contain added liquidity");
